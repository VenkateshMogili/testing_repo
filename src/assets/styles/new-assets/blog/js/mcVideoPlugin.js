...
var mcVc = function (d, c) {
  ...
  for (var b = document.getElementById(d).getElementsByTagName("div"), a = 0; a < b.length; a++) if (b[a].className == "mc-caption-bg" || b[a].className == "mc-caption-bg2") b[a].style.visibility = c ? "visible" : "hidden"
}, McVideo = function () {
  var a = [], b = function (b, e) {
    if (!b.vPlayer) for (var c = 0; c < a.length; c++) {
      var d = new a[c](b, e);
  ...
  var c = function () {
    var b;
    if (window.addEventListener) window.addEventListener("message", d, false); else window.attachEvent("onmessage", d, false);
    ...
    function e() {
      ...
      if (d == "replay") c("play"); else d != "false" && a.To(1, 1)
    }
  ...
  function e(f, k, j, e) {
    var g = "&loop=0&autoplay=1&wmode=opaque&color=bbbbbb&" + (new Date).getTime(), c = f.getAttribute("href"),
...
var YoutubePlayer = function (b, a) {
  ...
  var c = function () {
    ...
    function f(b) {
      if (b.data == 0) {
        var d = document.getElementById("mcVideo" + a.Id), c = d.parentNode.parentNode.getAttribute("data-autonext");
        if (c == "replay") b.target.d(); else c != "false" && a.To(1, 1);
        mcVc(a.Id, 1)
      }
      if (b.data == 1) mcVc(a.Id, 0); else b.data == 2 && mcVc(a.Id, 2)
    }
  ...
  function e(e, j, i, c) {
    var f = "&loop=0&start=0&wmode=opaque&autohide=1&showinfo=0&iv_load_policy=3&modestbranding=1&showsearch=0",
      b = e.getAttribute("href"), h = b.toLowerCase().indexOf("v="),
      g = '<iframe id="mcVideo' + c + '" src=' + location.protocol + "//www.youtube.com/embed/" + b.substring(h + 2) + "?enablejsapi=1&autoplay=1" + f + '" frameborder="0" width="' + j + '" height="' + i + '" allowfullscreen></iframe>',
...
var McVAHelper = {
  b: function (c) {
    var a = c.parentNode.getElementsByTagName("div"), b = a.length;
    while (b--) if (a[b].className == "sliderInner") {
      a[b].innerHTML = "";
      break
    }
  }, c: function () {
    ...
    if (a.style.display == "none") {
      if (this.c()) return 0;
      var b = a.getElementsByTagName("source"), d = b.length, f = 1;
      while (d--) if (!b[d].getAttribute("src")) {
        f = 0;
        b[d].setAttribute("src", b[d].getAttribute("data-src"))
      }
      a.style.display = "block";
      if (e == "image") a.style.background = c.parentNode.style.background; else c.parentNode.style.background = e;
    ...
    var b = function (a) {
      if (a && a.stopPropagation) a.stopPropagation(); else wdl.event.cancelBubble = true
